...
import helpers.helper_zz as helper_utils
import helpers.src_parser as source_parser
import sys
import pickle
import copy
import os
...
# Original function: get_real_file_diff
...
# Remaining functions will be obfuscated in the next steps
...
# Original function: save_commit_file
...
# Original function: get_mainfilecommits22
...
# Original function: get_mainfilecommits
...
# Original function: get_negative_file
...
def Retrieve_Initial_Commit(repo_directory=None):
    ...
    result = helper_utils.execute_command(command_line)
    first_commit = result[0][:8]  # Extract the first 8 characters of the first commit
    return first_commit

# Original function: commit_reset
def reset_commit(repo_path=None, commit_id=None):
    ...
    helper_utils.execute_command(reset_command)


# Original function: Projec_config_make
def configure_project_make(repo_path=None):
    ...
    for file in project_dirs:
        if file.lower() in ["config", "configure"] and os.path.isfile(repo_path + file):
            configure_file(repo_path + file, repo_path)
            if "Makefile" in os.listdir(repo_path):
                flag_return = 1
                break
    if flag_return:
        make_command = "cd " + repo_path + ";make -j10"
        helper_utils.execute_command(make_command)
    return flag_return

# Original function: config_pro
def execute_config_procedure(config_file_name=None, repo_directory=None):
    ...
    exec_result = helper_utils.execute_command(config_cmd)
    if "unrecognized option" in exec_result:
        config_cmd = 'cd ' + repo_directory + ';./' + config_file_name
        helper_utils.execute_command(config_cmd)

# Original function: delete_compilefile
def remove_compiled_files(compile_path=None, repo_directory=None):
    ...
    helper_utils.execute_command(clean_cmd)
...
# Original function: Patchinfo_Get2
